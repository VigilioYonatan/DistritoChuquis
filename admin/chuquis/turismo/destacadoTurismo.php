...
$queryDestacado = mysqli_query($cnx, "SELECT * FROM chuquis_tables WHERE ChuquisCod = 'CHU-TUR' AND destacado = 1");
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $buscarCodigo = mysqli_query($cnx, "SELECT * FROM chuquis_tables WHERE ChuquisCod = 'CHU-TUR' and cod = '$codigo'");
    ...
    if(empty($errores)){
        ...
        $borrarDestacado = mysqli_query($cnx, "UPDATE chuquis_tables SET destacado = 0 WHERE cod = '$codDestado' AND ChuquisCod = 'CHU-TUR'");
        $updDestacado = mysqli_query($cnx, "UPDATE chuquis_tables SET destacado = 1 WHERE cod = '$codigo' AND ChuquisCod = 'CHU-TUR'");

        if($updDestacado){
            header('Location: index.php?action=destacadoTurismo&actualizado=1');
        }
    }
}
?>
...
<section class="configuracion">
    ...
    <form action="" class="configuracion-form" method="POST">
        <h3 class="configuracion-title">Cambiar imagen destacado de Turismo</h2>
        <div class="configuracion-inp center">
            ...
            <?php echo isset($fotoDestado) ? "<img width='200px' src='./mediaBD/mediaChuquis/turismo/$fotoDestado'>" : ''; ?>
